...
location doesn't refetch immediately.
...
|---|---|---|
| `streamlit==1.50.0` | `streamlit` | the whole UI - form widgets, layout, caching (`st.cache_data`) |
...
| Open-Meteo (`api.open-meteo.com`) | latitude/longitude -> IANA timezone name (via `timezone=auto`) | public REST, no key - only the resolved timezone is used, not the weather data itself |

## Files

```
solunar/
  solunar.py          <- the whole app (UI + geocoding + solunar math)
  requirements.txt    <- streamlit, requests, ephem, tzdata
