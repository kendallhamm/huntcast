# Huntcaster — literature review and scoring decisions

This is the long-form companion to [README.md](README.md). It holds the
full reasoning behind every number in Huntcaster's scoring model, the
...
**The one-line version:** every activity weight is a measured effect
size from a GPS-collar study, expressed in yards per hour of excess
daytime buck movement, and nothing in the activity block is hand-tuned.
The weather terms could not be calibrated that way — no located study
reports weather effects as a movement rate — so they are bounded
judgment calls, and the bounds are set to preserve the evidential
ordering rather than to feel right.

**Conventions used below.** Each source entry states what this project
takes from it, flags where the source *disagrees* with the model, and
notes any figure that could only be checked against an abstract. Entries
marked **Audit note** are corrections made during the September 2026
verification pass. Entries marked **Correction note** record a claim an
earlier version of this project made that turned out to be wrong; they
are kept rather than deleted so the same mistake isn't made twice.
...
- **Rain and wind each get half of it**, at the same tier as each other,
  ...
  in exactly 1 of those 8 models. The Penn State storm analysis can't
  raise rain any higher - its two years disagree on *direction* (see
  Sources), so it supports neither the size nor the sign of a rain
...
### Model mechanics — the formulas and constants

The app's panel shows the headline formula; this is the full expansion,
with the constant name in `solunar.py` next to each value so a number in
the model can be traced to a line of code and from there to a source
above.

```
score = activity + rut + (cold + pressure - penalty) × d
```

Every term is evaluated **hour by hour** and averaged over the window's
`H = WINDOW_HOURS = 6` hours, then converted to points by the single
shared constant `k = POINTS_PER_YPH = 3.0 / 142 ≈ 0.0211`.

**1. Daily activity.**

```
activity = (k / H) × Σ_h Σ_e  y_e × o_(h,e)
```
...
```
cold_h = Y_cold × clamp(ΔT_h / S, 0, 1)
```

`ΔT_h` is how many °F below normal hour *h* runs, where "normal" is the
mean of the last `PAST_DAYS_LOOKBACK = 7` days of observations at the
same hour of day. `S = COLD_ANOMALY_SCALE = 15`°F;
`Y_cold = COLD_MAX_YPH = 16` yph (max 0.34 points), which is
`WEATHER_BOUND_YPH`, itself defined as `CREPUSCULAR_YPH × 2 / WINDOW_HOURS`
— literally "one dawn's worth of contribution", in code.

**4. Rain/wind penalty.**

```
penalty_h = Y_rain × (p_h / 100) + Y_wind × clamp((v_h − v₀) / (v₁ − v₀), 0, 1)
```

`p_h` is the hour's precipitation chance (%) and `v_h` its wind speed
(mph). `Y_rain = PRECIP_MAX_PENALTY_YPH = 8` yph,
`Y_wind = WIND_MAX_PENALTY_YPH = 8` yph,
`v₀ = WIND_PENALTY_THRESHOLD_MPH = 15`,
`v₁ = WIND_PENALTY_FULL_MPH = 40` mph. Wind does not engage at all below
15 mph.

**5. Pressure.** Split into the two things the evidence can actually
tell apart — a *change* in pressure versus a static *level*:

```
pressure_h = Y_band × 1[P_low ≤ P_h ≤ P_high]          (band)
           + { Y_drop        if ΔP_h ≥ ΔP_min           (falling)
               Y_drop,minor  if ΔP_minor ≤ ΔP_h < ΔP_min
               0             otherwise }
```
...
carries half the weather weight of a midday hour.

### Window search and the chart

The top `CANDIDATE_WINDOW_COUNT = 15` highest-scoring windows are found
by sliding the 6-hour window across every possible starting hour in the
forecast, then keeping only **non-overlapping** windows — best score
wins any overlap — so the results are genuinely different opportunities
rather than the same window shifted by an hour.

| Chart colour | Terms it contains | Can it go negative? |
|---|---|---|
| **Rut Phase** | 2 | No — floored at zero outside the rut |
| **Daily Activity** | 1 (dawn/dusk + solunar) | No |
| **Weather** | (3 + 5 − 4) × 6 | Yes — a wet, windy window pulls its bar below zero |

Bonuses stack above the zero line and penalties hang below it, so a
bar's height is **not** its score. The diamond on each bar marks the net
total (the three segments summed), which is what the ranking uses.

### Why cold is measured against a local normal

A flat threshold - "colder than 45°F" - doesn't generalize down the East
Coast: 38°F means something very different in Maine than in Georgia, and
deer respond to change from what they're acclimated to. The app requests
7 days of *past* hourly observations alongside the forecast and builds a
per-hour-of-day normal from them, so a daytime window is compared against
daytime history rather than a day/night average.

That same past-days fetch is also what gives the 24-hour pressure
lookback something to reach into, so **windows in the first 24 hours of
the forecast can score a pressure trend at all.**
...
- **Solunar windows are displayed prominently** despite scoring near
  zero, because hunters expect to see them.
